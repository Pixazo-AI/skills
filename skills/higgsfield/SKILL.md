...
Advanced AI capabilities for image and video generation. Note: produces a fixed ~5-second clip — length is not adjustable.
...
- `POST https://gateway.pixazo.ai/ai-model-api/v1/image-to-video`

**Sample request (primary operation)**

```bash
curl -X POST 'https://gateway.pixazo.ai/ai-model-api/v1/image-to-video' \
...
```js
const res = await fetch('https://gateway.pixazo.ai/ai-model-api/v1/image-to-video', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/ai-model-api/v1/image-to-video", headers=HEADERS, json={...}).json()
...
- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
