...
description: Image generation/editing with Image Converter API (by Pixazo) via the Pixazo API. TRIGGER when the user mentions "Image Converter" or "Image Converter API", or when the user asks to generate / make / create / edit / restyle an image and Image Converter is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# Image Converter API

Convert an image between HEIC, WebP, PNG, JPG, BMP, TIFF, AVIF and GIF in any direction. EXIF orientation is APPLIED before metadata is stripped, so phone photos come back upright instead of sideways — the whole of "my iPhone photo won’t upload anywhere". Transparency is flattened onto WHITE, not black, when the target format has no alpha channel.

You can ask Image Converter to handle image generation/editing. Powered by Pixazo via the Pixazo API gateway.
...
|---|---|---|
| Image Converter 1.0 | Convert Image | `media-convert-image` / `media-convert-image-request` |

### Step 3 — Make the API call

**Endpoints**

- `POST https://gateway.pixazo.ai/media-tools/v1/image-convert`

**Sample request (primary operation)**

```bash
curl -X POST 'https://gateway.pixazo.ai/media-tools/v1/image-convert' \
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/media-tools/v1/image-convert",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/media-tools/v1/image-convert', {
  method: 'POST',
  headers: {
