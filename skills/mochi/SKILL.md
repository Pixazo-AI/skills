---
name: mochi
description: Video generation with Mochi Video API (by Mochi) via the Pixazo API. TRIGGER when the user mentions "Mochi" or "Mochi Video API", or when the user asks to generate / make / create a video / clip / animation and Mochi is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# Mochi Video API

Video generation capabilities. Note: produces a fixed ~5-second clip — length is not adjustable, and the price is a flat per-video charge.

You can ask Mochi to handle video generation. Powered by Mochi via the Pixazo API gateway.
...
|---|---|---|
| Mochi v1 | Text to Video | `68a5784c0828c041ba519ca6` / `submit-video-request` |
...
```bash
curl -X POST 'https://gateway.pixazo.ai/mochi-v1-clone/generate' \
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/mochi-v1-clone/generate",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/mochi-v1-clone/generate', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/mochi-v1-clone/generate", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/mochi-v1-clone/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/mochi.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/mochi`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
