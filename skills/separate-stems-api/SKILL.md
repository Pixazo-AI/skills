---
name: separate-stems-api
description: Image generation/editing with Audio Stem Separation API (by Pixazo) via the Pixazo API. TRIGGER when the user mentions "Audio Stem Separation" or "Audio Stem Separation API", or when the user asks to generate / make / create / edit / restyle an image and Audio Stem Separation is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# Audio Stem Separation API

Split an audio or video file into isolated stems. Two-stem mode returns vocals and no_vocals — the pairing dubbing needs, so replacing speech no longer destroys the music bed. Four-stem mode returns vocals, drums, bass and other for remixing. Pass an audio_url (a video URL is fine; the audio is extracted server-side), poll the request, then download the stems as MP3 or WAV.

You can ask Audio Stem Separation to handle image generation/editing. Powered by Pixazo via the Pixazo API gateway.
...
```js
const res = await fetch('https://gateway.pixazo.ai/audio-tools/v1/separate-stems', {
  method: 'POST',
  headers: {
