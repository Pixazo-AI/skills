---
name: bria
description: Image generation/editing with Bria API (by Bria) via the Pixazo API. TRIGGER when the user mentions "Bria" or "Bria API", or when the user asks to generate / make / create / edit / restyle an image and Bria is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---
...
- **Other image generation/editing models:** `seedream`, `flux`, `gpt-image`, `grok-imagine-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `studio-ghibli`, `auraflow`, `z-image`, `sdxl`, `firered-image-edit`, `codeformer`, `gfpgan`, `smart-resize`, `nucleus`, `glm-image`, `hidream`, `ernie-image`, `mirelo`, `real-esrgan`
