---
name: video-to-previs
description: Video generation with Video to Previs (by Pixazo) via the Pixazo API. TRIGGER when the user mentions "Video to Previs", or when the user asks to generate / make / create a video / clip / animation and Video to Previs is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# Video to Previs

Shot2Previs converts a video shot into a temporally consistent previs (previsualization) reference — a clay-shaded 3D depth render with multi-person pose skeletons that captures the shot's camera movement, framing, and subject blocking without reproducing any pixels. Pass a video URL, poll the request, download the previs mp4; feed it to a generative video model (Seedance, Wan-VACE, LTX) as a structural reference. Optional green-screen mode isolates the performer.

You can ask Video to Previs to handle video generation. Powered by Pixazo via the Pixazo API gateway.
...
|---|---|---|
| Video to Previs | Video to Video (Previs — Depth + Pose Reference) | `shot2previs` / `shot2previs-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/shot2previs/v1/video-to-video', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/shot2previs/v1/video-to-video", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/shot2previs/v1/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/video-to-previs.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/video-to-previs`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `magi`, `vibeo`
