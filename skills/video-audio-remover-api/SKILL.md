---
name: video-audio-remover-api
description: Image generation/editing with Video Audio Remover API (by Pixazo) via the Pixazo API. TRIGGER when the user mentions "Video Audio Remover" or "Video Audio Remover API", or when the user asks to generate / make / create / edit / restyle an image and Video Audio Remover is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# Video Audio Remover API

Remove the soundtrack from a video and get back a silent MP4 at the original resolution, frame rate and duration. H.264 sources are stream-copied so the picture is untouched; other codecs are re-encoded to H.264. Sources up to about 35 MB.

You can ask Video Audio Remover to handle image generation/editing. Powered by Pixazo via the Pixazo API gateway.
...
|---|---|---|
| Video Audio Remover 1.0 | Strip Audio | `media-strip-audio` / `media-strip-audio-request` |

### Step 3 — Make the API call

**Endpoints**

- `POST https://gateway.pixazo.ai/media-tools/v1/video-strip-audio`

**Sample request (primary operation)**

```bash
curl -X POST 'https://gateway.pixazo.ai/media-tools/v1/video-strip-audio' \
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/media-tools/v1/video-strip-audio",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/media-tools/v1/video-strip-audio', {
  method: 'POST',
  headers: {
