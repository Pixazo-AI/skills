---
name: sora
description: Video generation with Sora Video API (by OpenAI) via the Pixazo API. TRIGGER when the user mentions "Sora" or "Sora Video API", or when the user asks to generate / make / create a video / clip / animation and Sora is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---
...
|---|---|---|
| Sora 2 Pro | Image to Video | `sora-video` / `generate-image-to-video-request` |
| Sora 2 Pro | Text to Video | `sora-video` / `generate-video-request` |
...
```bash
curl -X POST 'https://gateway.pixazo.ai/sora-video/v1/video/i2v/generate' \
...
```js
const res = await fetch('https://gateway.pixazo.ai/sora-video/v1/video/i2v/generate', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/sora-video/v1/video/i2v/generate", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/sora-video/v1/video/i2v/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/sora.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/sora`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
