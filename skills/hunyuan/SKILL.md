---
name: hunyuan
description: 3D model generation with Hunyuan AI API (by Tencent) via the Pixazo API. TRIGGER when the user mentions "Hunyuan" or "Hunyuan AI API", or when the user asks to generate / make a 3D model / mesh / asset and Hunyuan is named or implied. DO NOT TRIGGER for image / video / music / voice / try-on — each has its own skill.
---

# Hunyuan AI API

Advanced AI capabilities by Tencent including image and 3D generation.

You can ask Hunyuan to handle 3D model generation. Powered by Tencent via the Pixazo API gateway.
...
|---|---|---|
| Hunyuan v3.0 | Text to Image | `hunyuan-image` / `generate-image-request` |
| Hunyuan v3.0 | Image to Image (3D Models — Image to 3D) | `hunyuan3d-3-0-api-294` / `hunyuan3d-3-0-api-request` |
...
```bash
curl -X POST 'https://gateway.pixazo.ai/hunyuan-image/v1/hunyuan-image/generateRequest' \
...
```js
const res = await fetch('https://gateway.pixazo.ai/hunyuan-image/v1/hunyuan-image/generateRequest', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/hunyuan-image/v1/hunyuan-image/generateRequest", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/hunyuan-image/v1/hunyuan-image/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/hunyuan.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/hunyuan`.

---

## Related Pixazo skills

- **Other 3D model generation models:** `hyper3d`, `tripo3d`, `trellis3d`
