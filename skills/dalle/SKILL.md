---
name: dalle
description: Image generation/editing with DALL-E API (by OpenAI) via the Pixazo API. TRIGGER when the user mentions "Dalle" or "DALL-E API", or when the user asks to generate / make / create / edit / restyle an image and Dalle is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# DALL-E API

DALL-E API by OpenAI.

You can ask Dalle to handle image generation/editing. Powered by OpenAI via the Pixazo API gateway.
...
> **Fetch:** `https://www.pixazo.ai/models/dalle.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/dalle`.

---

## Related Pixazo skills

- **Other image generation/editing models:** `seedream`, `flux`, `gpt-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `studio-ghibli`, `auraflow`, `z-image`, `bria`, `sdxl`, `firered-image-edit`
