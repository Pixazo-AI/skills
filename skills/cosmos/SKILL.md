---
name: cosmos
description: Video generation with Cosmos API (by NVIDIA) via the Pixazo API. TRIGGER when the user mentions "Cosmos" or "Cosmos API", or when the user asks to generate / make / create a video / clip / animation and Cosmos is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---
...
|---|---|---|
| Cosmos 3 Super | Text to Image | `cosmos-3-super-text-to-image` / `cosmos-3-super-text-to-image-request` |
| Cosmos 3 Super | Image to Video | `cosmos-3-super-image-to-video` / `cosmos-3-super-image-to-video-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/cosmos-3-super-text-to-image/v1/cosmos-3-super-text-to-image-request', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/cosmos-3-super-text-to-image/v1/cosmos-3-super-text-to-image-request", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/cosmos-3-super-text-to-image/v1/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/cosmos.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/cosmos`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `video-to-previs`, `magi`, `vibeo`
