---
name: z-image
description: Image generation/editing with Z-Image API (by Z-Image) via the Pixazo API. TRIGGER when the user mentions "Z Image" or "Z-Image API", or when the user asks to generate / make / create / edit / restyle an image and Z Image is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---
...
You can ask Z Image to handle image generation/editing. Powered by Z-Image via the Pixazo API gateway.
...
|---|---|---|
| Z-Image Turbo | Text to Image | `z-image-turbo-834` / `z-image-turbo-request` |
| Z-Image Base | Text to Image | `z-image-base` / `z-image-base-request` |
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/z-image-turbo-834/v1/z-image-turbo-request",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/z-image-turbo-834/v1/z-image-turbo-request', {
  method: 'POST',
  headers: {
...
> **Fetch:** `https://www.pixazo.ai/models/z-image.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/z-image`.

---

## Related Pixazo skills

- **Other image generation/editing models:** `seedream`, `flux`, `gpt-image`, `grok-imagine-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `studio-ghibli`, `auraflow`, `bria`, `sdxl`, `firered-image-edit`, `codeformer`, `gfpgan`, `smart-resize`, `nucleus`, `glm-image`, `hidream`, `ernie-image`, `mirelo`, `real-esrgan`
