---
name: auraflow
description: Image generation/editing with Auraflow API (by Auraflow) via the Pixazo API. TRIGGER when the user mentions "Auraflow" or "Auraflow API", or when the user asks to generate / make / create / edit / restyle an image and Auraflow is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---
...
You can ask Auraflow to handle image generation/editing. Powered by Auraflow via the Pixazo API gateway.
...
|---|---|---|
| Auraflow v0.3 | Text to Image | `auraflow-v0-3-512` / `auraflow-v0-3-request` |
...
```bash
curl -X POST 'https://gateway.pixazo.ai/auraflow-v0-3-512/v1/auraflow-v0-3-request' \
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/auraflow-v0-3-512/v1/auraflow-v0-3-request",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/auraflow-v0-3-512/v1/auraflow-v0-3-request', {
  method: 'POST',
  headers: {
...
> **Fetch:** `https://www.pixazo.ai/models/auraflow.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/auraflow`.

---

## Related Pixazo skills

- **Other image generation/editing models:** `seedream`, `flux`, `gpt-image`, `grok-imagine-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `studio-ghibli`, `z-image`, `bria`, `sdxl`, `firered-image-edit`, `codeformer`, `gfpgan`, `smart-resize`, `nucleus`, `glm-image`, `hidream`, `ernie-image`, `mirelo`, `real-esrgan`
