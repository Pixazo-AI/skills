---
name: omnihuman
description: Video generation with OmniHuman API (by BytePlus) via the Pixazo API. TRIGGER when the user mentions "OmniHuman" or "OmniHuman API", or when the user asks to generate / make / create a video / clip / animation and OmniHuman is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# OmniHuman API

Advanced AI lipsync and talking video generation by ByteDance.

You can ask OmniHuman to handle video generation. Powered by BytePlus via the Pixazo API gateway.
...
|---|---|---|
| OmniHuman v1.5 | Audio to Video (Ref Image + Ref Audio to Video — Lipsync) | `bytedance-omnihuman-v1-5-290` / `bytedance-omnihuman-v1-5-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/bytedance-omnihuman-v1-5-290/v1/bytedance-omnihuman-v1-5-request', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/bytedance-omnihuman-v1-5-290/v1/bytedance-omnihuman-v1-5-request", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/bytedance-omnihuman-v1-5-290/v1/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/omnihuman.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/omnihuman`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
