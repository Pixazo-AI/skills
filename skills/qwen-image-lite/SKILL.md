---
name: qwen-image-lite
description: Image generation/editing with Qwen Image Edit API (by Alibaba) via the Pixazo API. TRIGGER when the user mentions "Qwen Image Edit" or "Qwen Image Edit API", or when the user asks to generate / make / create / edit / restyle an image and Qwen Image Edit is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# Qwen Image Edit API

Qwen-Image-Edit-2509 (Lightning 4-step) - fast, free image editing. Send a source image plus a text instruction and get the edited result back.

You can ask Qwen Image Edit to handle image generation/editing. Powered by Alibaba via the Pixazo API gateway.
...
|---|---|---|
| Qwen Image Edit | Image to Image (Image Editing) | `qwen-image-lite` / `qwen-image-lite-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/qwen-image-lite/v1/qwen-image-lite-request', {
  method: 'POST',
  headers: {
...
> **Fetch:** `https://www.pixazo.ai/models/qwen-image-lite.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/qwen-image-lite`.
