---
name: krea
description: Image generation/editing with Krea API (by Krea.AI) via the Pixazo API. TRIGGER when the user mentions "Krea" or "Krea API", or when the user asks to generate / make / create / edit / restyle an image and Krea is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---

# Krea API

Advanced text-to-image generation by Krea.AI featuring reference images, LoRA style presets, and moodboard conditioning.

You can ask Krea to handle image generation/editing. Powered by Krea.AI via the Pixazo API gateway.
...
|---|---|---|
| Krea 2 Turbo | Text to Image | `krea-2-turbo` / `krea-2-turbo-request` |
| Krea 2 Turbo | Text to Image (LoRA) | `krea-2-turbo-lora` / `krea-2-turbo-lora-request` |
| Krea 2 Large | Text to Image | `krea-2-large` / `text-to-image` |
| Krea 2 Large | Krea 2 Large Fast | `krea-2-large-fast` / `text-to-image` |
| Krea 2 Large | Image to Image (LoRA Trainer) | `krea-2-trainer` / `krea-2-trainer-request` |
...
> **Fetch:** `https://www.pixazo.ai/models/krea.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/krea`.

---

## Related Pixazo skills

- **Other image generation/editing models:** `seedream`, `gpt-image`, `grok-imagine-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `stable-diffusion`, `studio-ghibli`, `auraflow`, `z-image`, `bria`, `sdxl`, `firered-image-edit`, `codeformer`, `gfpgan`, `smart-resize`, `nucleus`, `glm-image`, `hidream`, `ernie-image`, `mirelo`, `real-esrgan`, `mai-image`, `pixelcut`, `boogu-image`, `whisper`, `assemblyai`, `separate-stems-api`, `diarize-api`, `video-convert-api`, `video-crop-api`
