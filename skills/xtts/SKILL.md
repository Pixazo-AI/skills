...
|---|---|---|
| XTTS 2 | Text to Speech (Voice Cloning) | `xtts-v2-api` / `text-to-speech-request` |
...
```bash
curl -X POST 'https://gateway.pixazo.ai/voice-clone/v1/xtts-v2/generate' \
...
import os, requests
r = requests.post(
    "https://gateway.pixazo.ai/voice-clone/v1/xtts-v2/generate",
    headers={
...
```js
const res = await fetch('https://gateway.pixazo.ai/voice-clone/v1/xtts-v2/generate', {
  method: 'POST',
  headers: {
...
> **Fetch:** `https://www.pixazo.ai/models/xtts.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/xtts`.

---

## Related Pixazo skills

- **Other text-to-speech / voice synthesis models:** `chatterbox`, `vibevoice`, `elevenlabs`, `gemini`, `qwen-audio`, `gemini-voice`, `voxcpm`, `zonos`, `fish-audio`, `deepgram`, `inworld`, `xai-tts`, `lux-tts`, `tada`, `melotts`, `gpt-4o`, `seed-audio`, `mai-voice`, `gpt-live`
