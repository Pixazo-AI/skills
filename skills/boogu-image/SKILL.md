---
name: boogu-image
description: Image generation/editing with Boogu Image API (by Boogu) via the Pixazo API. TRIGGER when the user mentions "Boogu Image" or "Boogu Image API", or when the user asks to generate / make / create / edit / restyle an image and Boogu Image is named or implied. DO NOT TRIGGER for video / music / voice / 3d / try-on — each has its own skill.
---
...
You can ask Boogu Image to handle image generation/editing. Powered by Boogu via the Pixazo API gateway.
...
|---|---|---|
| Boogu Image v1 | Text to Image | `boogu-image` / `boogu-image-request` |
| Boogu Image v1 | Image to Image (Image Editing) | `boogu-image-edit` / `boogu-image-edit-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/boogu-image/v1/boogu-image-request', {
  method: 'POST',
  headers: {
...
> **Fetch:** `https://www.pixazo.ai/models/boogu-image.md`

Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/boogu-image`.

---

## Related Pixazo skills

- **Other image generation/editing models:** `seedream`, `gpt-image`, `grok-imagine-image`, `ideogram`, `longcat-image`, `nano-banana`, `pixelforge`, `qwen-image`, `recraft`, `reve-image`, `stable-diffusion`, `studio-ghibli`, `auraflow`, `z-image`, `bria`, `sdxl`, `firered-image-edit`, `codeformer`, `gfpgan`, `smart-resize`, `nucleus`, `glm-image`, `hidream`, `ernie-image`, `mirelo`, `real-esrgan`, `mai-image`, `pixelcut`, `krea`, `whisper`, `assemblyai`, `separate-stems-api`, `diarize-api`, `video-convert-api`, `video-crop-api`
