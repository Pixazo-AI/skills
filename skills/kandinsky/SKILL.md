---
name: kandinsky
description: Video generation with Kandinsky API (by Kandinsky) via the Pixazo API. TRIGGER when the user mentions "Kandinsky" or "Kandinsky API", or when the user asks to generate / make / create a video / clip / animation and Kandinsky is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# Kandinsky API

AI-powered image-to-video generation by Kandinsky.

You can ask Kandinsky to handle video generation. Powered by Kandinsky via the Pixazo API gateway.
...
> **Fetch:** `https://www.pixazo.ai/models/kandinsky.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/kandinsky`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
