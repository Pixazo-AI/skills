---
name: trellis3d
description: 3D model generation with Trellis 3D API (by Trellis) via the Pixazo API. TRIGGER when the user mentions "Trellis3d" or "Trellis 3D API", or when the user asks to generate / make a 3D model / mesh / asset and Trellis3d is named or implied. DO NOT TRIGGER for image / video / music / voice / try-on — each has its own skill.
---
...
You can ask Trellis3d to handle 3D model generation. Powered by Trellis via the Pixazo API gateway.
...
```js
const res = await fetch('https://gateway.pixazo.ai/trellis-2-image-to-3d/v1/trellis-2-image-to-3d-request', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/trellis-2-image-to-3d/v1/trellis-2-image-to-3d-request", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/trellis-2-image-to-3d/v1/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/trellis3d.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/trellis3d`.

---

## Related Pixazo skills

- **Other 3D model generation models:** `hunyuan`, `hyper3d`, `tripo3d`, `meshy-3d`, `pixal3d`
