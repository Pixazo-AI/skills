---
name: lucy-edit
description: Video generation with Lucy Edit Video API (by Decart) via the Pixazo API. TRIGGER when the user mentions "Lucy Edit" or "Lucy Edit Video API", or when the user asks to generate / make / create a video / clip / animation and Lucy Edit is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# Lucy Edit Video API

Fast video editing capabilities.

You can ask Lucy Edit to handle video generation. Powered by Decart via the Pixazo API gateway.
...
|---|---|---|
| Lucy Edit Pro | Video to Video (Video Editing) | `decart-lucy-edit-video-fast-142` / `decart-lucy-edit-video-fast-request` |
...
```js
const res = await fetch('https://gateway.pixazo.ai/decart-lucy-edit-video-fast-142/v1/decart-lucy-edit-video-fast-request', {
  method: 'POST',
  headers: {
...
# 1) Submit
submit = requests.post("https://gateway.pixazo.ai/decart-lucy-edit-video-fast-142/v1/decart-lucy-edit-video-fast-request", headers=HEADERS, json={...}).json()
...
while True:
    status = requests.get(f"https://gateway.pixazo.ai/decart-lucy-edit-video-fast-142/v1/result/{task_id}", headers=HEADERS).json()
...
> **Fetch:** `https://www.pixazo.ai/models/lucy-edit.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/lucy-edit`.

---

## Related Pixazo skills

- **Other video generation models:** `sync-lipsync`, `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `ltx`, `luma`, `mochi`, `veed`, `vidu`, `wan`, `minimax`, `pixverse`, `kandinsky`, `hunyuan-video`, `heygen`, `grok-imagine-video`, `gemini-omni`, `gemini-omni-1-1`, `cosmos`, `video-to-previs`, `magi`, `vibeo`
