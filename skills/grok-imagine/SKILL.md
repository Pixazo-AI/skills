---
name: grok-imagine
description: Video generation with Grok Imagine API (by xAI) via the Pixazo API. TRIGGER when the user mentions "Grok" or "Grok Imagine API", or when the user asks to generate / make / create a video / clip / animation and Grok is named or implied. DO NOT TRIGGER for image / music / voice / 3d / try-on — each has its own skill.
---

# Grok Imagine API

Image generation by xAI.

You can ask Grok to handle video generation. Powered by xAI via the Pixazo API gateway.
...
> **Fetch:** `https://www.pixazo.ai/models/grok-imagine.md`
...
Load that URL when you need exact parameter names, accepted values, or aren't sure about a field. The HTML version is at `https://www.pixazo.ai/models/grok-imagine`.

---

## Related Pixazo skills

- **Other video generation models:** `happy-horse`, `p-video`, `seedance`, `sora`, `veo`, `runway`, `kling`, `pika`, `higgsfield`, `genflare`, `omnihuman`, `lucy-edit`, `ltx`, `luma`, `hailuo`, `mochi`, `stable-diffusion`, `veed`, `vidu`, `wan`, `pixverse`, `kandinsky`, `hunyuan-video`
